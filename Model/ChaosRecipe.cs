...
namespace Recipe.Model
{
    public class ChaosRecipe : IRecipe
    {
        ...
        private List<RecipeItem> _currentSet;
        private List<TabIndexItemTypes> _tabIndexItemTypes;
        private StashElement _stashElement => _gameController.Game.IngameState.IngameUi.StashElement;
        ...
        private void UpdateTabIndexItemTypes(object sender, int i)
        {
            _tabIndexItemTypes = new List<TabIndexItemTypes>
           {
               ...
               new TabIndexItemTypes(ItemType.OneHandedWeapon, _settings.Weapons.Value),
               new TabIndexItemTypes(ItemType.TwoHandedWeapon, _settings.Weapons.Value),
        ...
        {
            foreach (var recipeItem in _currentSet.OrderBy(x => x.ItemType))
            {
                var stashIndex = GetStashIndexOfRecipeItem(recipeItem);
                // GoToTab(stashIndex)
                var inventoryItems = GetStashInventory(stashIndex).VisibleInventoryItems.ToList();
                var inventoryItem = FindRecipeItemInStashTab(recipeItem, inventoryItems);
            }
            throw new System.NotImplementedException();
        }

        private NormalInventoryItem FindRecipeItemInStashTab(RecipeItem recipeItem, List<NormalInventoryItem> inventoryItems)
        {
            var inventoryItem = inventoryItems.Find(x => Equals(x, recipeItem.NormalInventoryItem));
        ...
        {
                var stashIndex = _tabIndexItemTypes.FirstOrDefault(x => x.ItemType == recipeItem.ItemType);
                if (stashIndex == null)
                {
                    throw new Exception("Couldn't find stash index of item type.");
                }
